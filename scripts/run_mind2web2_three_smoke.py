...
CASES = [
    {
        ...
        "expect_any": ["44b5506", "2023-12-07", "younesbelkada", "ArthurZucker"],
    },
    {
        "id": "yu_lineage",
...
async def main() -> int:
    out_dir = Path("experiment_logs/mind2web2_three_smoke") / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    out_dir.mkdir(parents=True, exist_ok=True)
    orch = Orchestrator(time_limit_s=300)
    rows = []
    for case in CASES:
